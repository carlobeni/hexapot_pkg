...
# ============================================================
class LateralCrabGait(BaseGait):
    def __init__(self, node, direction=+1):
        joints = [
            "j_c1_rf","j_thigh_rf","j_tibia_rf",
            "j_c1_rm","j_thigh_rm","j_tibia_rm",
            "j_c1_rr","j_thigh_rr","j_tibia_rr",
        ...
        # postura base paralela (correcta)
        self.base_c1 = {
            "lf": +math.pi/4,
            "rf": -math.pi/4,
            "lm":  0.0,
        ...
        self.step = 0.25 * direction
        self.step_height = 0.7
        self.tibia_gain = 0.7   # <<< CLAVE

        self.T = 0.6
        self.start_time = node.get_clock().now()

    def pose(self, leg, c1, tibia):
        return {
            f"j_c1_{leg}": c1,
            f"j_thigh_{leg}": 0.0,
            f"j_tibia_{leg}": tibia
    ...
    def lateral_push(self, leg, push):
        return self.pose(
            leg,
            self.base_c1[leg] + push,
            -self.tibia_gain * push
        )
    ...
    def update(self):
        ...
        push = self.step * math.sin(2 * math.pi * phi)

        # === patas delanteras: ANCLAJE (ligeramente compensado) ===
        for l in ("lf", "rf"):
            P.update(self.pose(l, self.base_c1[l], 0.0))

        # === patas traseras: EMPUJE LATERAL REAL ===
        for l in ("lr", "rr"):
            P.update(self.lateral_push(l, push))

        # === patas medias alternan ===
        if phi < 0.5:
            # LM empuja, RM swing
            P.update(self.lateral_push("lm", push))
            P.update(self.pose(
                "rm",
                self.base_c1["rm"],
                self.step_height * math.sin(math.pi * phi * 2)
            ))
        else:
            # RM empuja, LM swing
            P.update(self.lateral_push("rm", push))
            P.update(self.pose(
                "lm",
                self.base_c1["lm"],
                self.step_height * math.sin(math.pi * (phi - 0.5) * 2)
            ))
...
class LateralLeftGait(LateralCrabGait):
    def __init__(self, node):
        super().__init__(node, direction=+1)

class LateralRightGait(LateralCrabGait):
    def __init__(self, node):
        super().__init__(node, direction=-1)
